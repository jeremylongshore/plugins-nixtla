...
DEFAULT_CONFIG = {
    ...
    'comparison': {
        'output': {
            'results_path': 'forecasting/results/comparison_metrics.csv'
        }
    }
}
...
Frequency codes:
  H = Hourly, D = Daily, W = Weekly, M = Monthly, Q = Quarterly, Y = Yearly
    ...
    parser.add_argument('--freq', required=True,
                       help='Data frequency (H, D, W, M, Q, Y)')
    ...
    parser.add_argument('--finetune-loss', choices=['mae', 'mse', 'rmse', 'mape'],
                       help='Loss function (default: mae)')
    parser.add_argument('--artifacts-dir',
                       help='Artifacts output directory')
    parser.add_argument('--output', default='forecasting/finetune_config.yml',
