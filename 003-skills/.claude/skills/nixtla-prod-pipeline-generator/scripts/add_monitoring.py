...
class MonitoringInjector:
    ...
    def add_monitoring(
        ...
        modified_code = self._add_monitoring_functions(
            modified_code, metrics, thresholds
        )
...
'''
    ...
    def _inject_monitoring_calls(self, code: str, metrics: List[str]) -> str:
        ...
        if not modified:
            logger.warning(
                "No monitor function found. "
                "Add monitoring calls manually after forecast step."
            )

        return code

    def _enhance_monitor_function(
        self,
        code: str,
        pattern: str,
        metrics: List[str]
...
Examples:
  ...
  python add_monitoring.py --pipeline pipelines/forecast_dag.py \\
    ...
    parser.add_argument(
        '--pipeline',
        required=True,
        help='Path to pipeline file to instrument'
    )

    parser.add_argument(
        '--metrics',
        required=True,
        help='Comma-separated metrics to track (e.g., smape,mase,mae)'
    )

    parser.add_argument(
        '--thresholds',
        help='Metric thresholds (e.g., smape=20,mase=1.5)'
    )

    parser.add_argument(
        '--alerts',
        help='Alert channels (e.g., email,slack,pagerduty)'
    )

    parser.add_argument(
        '--output',
        help='Output path (default: overwrite with backup)'
    )
