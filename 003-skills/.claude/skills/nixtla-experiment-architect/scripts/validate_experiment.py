...
def validate_config_structure(config: Dict[str, Any]) -> List[str]:
    ...
    if "horizon" not in config["forecast"]:
        ...
        errors.append(f"Invalid horizon: must be positive integer, got {config['forecast']['horizon']}")
    ...
    if "method" in config["cv"] and config["cv"]["method"] not in ["rolling", "expanding"]:
        errors.append(f"Invalid CV method: {config['cv']['method']} (must be 'rolling' or 'expanding')")
...
def validate_data_file(config: Dict[str, Any], verbose: bool = False) -> List[str]:
    ...
    try:
        ...
        if df[y_col].isnull().any():
            ...
            if null_pct > 50:
                errors.append(f"Target column '{y_col}' contains {null_pct:.1f}% null values (too high)")
            elif verbose:
