...
def create_contract(
    ...
    if exog_vars:
        ...
        for var in exog_vars:
            exog_rows.append(f"| `{var['name']}` | {var['dtype']} | [Description] | [Range/Values] |")
...
Examples:
  ...
  python create_contract.py --input data/sales.csv --id_col store_id --date_col date --target_col revenue
    ...
    parser.add_argument(
        '--mapping',
        help='Path to JSON mapping file from analyze_schema.py'
    )
    parser.add_argument(
        '--input',
    ...
    parser.add_argument(
        '--id_col',
        help='Column name for series identifier'
    )
    parser.add_argument(
        '--date_col',
    ...
    if not args.mapping and not (args.input and args.date_col and args.target_col):
        print("Error: Must provide either --mapping or (--input --date_col --target_col)", file=sys.stderr)
        parser.print_help()
        sys.exit(1)
