...
def analyze_schema(
    ...
    if not detected_date_col:
        raise ValueError(
            "No timestamp column detected. Specify manually with --date_col"
        )
    ...
    if not detected_target_col:
        raise ValueError(
            "No target column detected. Specify manually with --target_col"
        )
    ...
    analysis = {
        ...
        'exogenous_variables': [
            {'name': name, 'dtype': dtype} for name, dtype in exog_vars
        ]
    }
...
Examples:
  ...
  python analyze_schema.py --input data/sales.csv --output mapping.json
    ...
    parser.add_argument(
        '--id_col',
        help='Column name for series identifier (auto-detected if not specified)'
    )
    parser.add_argument(
        '--date_col',
        help='Column name for timestamp (auto-detected if not specified)'
    )
    parser.add_argument(
        '--target_col',
        help='Column name for target variable (auto-detected if not specified)'
    )
    parser.add_argument(
        '--output',
        help='Output path for JSON mapping file (prints to stdout if not specified)'
    )
    ...
    try:
        ...
        print(f"  Target range: {stats['target_range']['min']:.2f} to {stats['target_range']['max']:.2f} (mean: {stats['target_range']['mean']:.2f})")
