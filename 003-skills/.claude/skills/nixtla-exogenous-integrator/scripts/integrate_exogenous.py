...
def align_data(df: pd.DataFrame, exog_df: Optional[pd.DataFrame]) -> pd.DataFrame:
    ...
    if len(common_columns - {"ds", "unique_id", "y"}) > 0:
        raise ValueError(
            f"Overlapping column names (excluding 'ds', 'unique_id', and 'y') found in both dataframes: {common_columns - {'ds', 'unique_id', 'y'}}"
        )
...
def generate_forecast(
    df: pd.DataFrame, h: int, freq: str, exog_cols: Optional[List[str]] = None
) -> pd.DataFrame:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate time series forecast with exogenous variables using TimeGPT."
    )
    parser.add_argument("--input", required=True, help="Path to the historical data CSV file.")
    parser.add_argument(
        "--exogenous", required=False, help="Path to the exogenous variables CSV file."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Frequency of the time series (e.g., 'D' for daily, 'H' for hourly).",
    )
