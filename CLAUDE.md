...
`004-scripts/validate_skills_v2.py` is synced from `claude-code-plugins` (`scripts/validate-skills-schema.py`). To update, run:

```bash
cd /home/jeremy/000-projects/claude-code-plugins && ./scripts/sync-validators-to-nixtla.sh
```

**Two-tier flags:**
- `--standard` (default) - No required fields enforced; suitable for local/experimental skills
- `--enterprise` - Marketplace-grade validation with all required fields enforced
