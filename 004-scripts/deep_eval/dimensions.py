...
ANTI_PATTERNS = [
    ...
    (
        re.compile(r"(?:^|\n)\s*\.\.\.\s*(?:\n|$)"),
        "ellipsis",
        "Contains bare ellipsis (truncated content)",
    ),
    (
        re.compile(
            r"\bmust never\b.*\bmust always\b|\bmust always\b.*\bmust never\b",
...
def score_orchestration_fitness(
    ...
    if orch_hits:
        ...
        score -= penalty
        details.append(
            f'Orchestration signals found ({len(orch_hits)} hits): {", ".join(orch_hits[:3])}'
        )
    else:
    ...
    if step_count > 5:
        score -= 10
        details.append(
            f"Multi-step workflow ({step_count} steps) — consider splitting into multiple skills"
        )
...
def score_robustness(
    ...
    has_error_section = bool(
        re.search(r"^##\s+Error\s+Handling", body, re.MULTILINE | re.IGNORECASE)
    )
    ...
    if total_blocks == 0:
        ...
        if code_content:
            ...
            if 3 <= avg_lines <= 30:
                ...
                score += 15
                details.append(
                    f"Code blocks avg {avg_lines:.0f} lines (consider moving to references/)"
                )

        # Bash examples for CLI-oriented skills
        bash_blocks = sum(
            1 for lang in code_blocks if lang and lang.lower() in ("bash", "sh", "shell", "zsh")
        )
        if bash_blocks > 0:
            score += 15
