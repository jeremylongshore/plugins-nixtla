...
def format_terminal(
    ...
    lines.append(
        f"LLM layer:        {'Active' if summary.get('llm_available') else 'Inactive (static only)'}"
    )
    lines.append("")
    ...
    if alignment.get("aligned", 0) + alignment.get("divergent", 0) > 0:
        ...
        lines.append(
            f"Grade/Badge alignment: {alignment['aligned']}/{total_compared} ({align_pct:.0f}%)"
        )
        lines.append("")
    ...
    if verbose:
        ...
        for r in sorted_results:
            ...
            lines.append(
                f"  {emoji} {name:40s}  deep={score:5.1f}  det={det_score}  ({elapsed:.1f}s)"
            )
            ...
            if static_dims and verbose:
                ...
                for dim, data in sorted(
                    static_dims.items(), key=lambda x: x[1].get("weight", 0), reverse=True
                ):
            ...
            if anti and anti.get("count", 0) > 0:
                lines.append(
                    f"      anti-patterns: {anti['count']} ({', '.join(h['name'] for h in anti['hits'][:3])})"
                )
        lines.append("")
