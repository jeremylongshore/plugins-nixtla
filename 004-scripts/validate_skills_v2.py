...
OPTIONAL_FIELDS = (
    set(SKILL_FIELDS.keys()) | set(INVALID_SKILL_FIELDS.keys()) | set(DEPRECATED_FIELDS.keys())
)
...
def score_progressive_disclosure(path: Path, body: str, fm: dict) -> dict:
    ...
    if lines <= 100:
        ...
        breakdown["navigation_signals"] = (
            2,
            f"Minimal structure: {sections} sections (4+ recommended)",
        )
    else:
...
def score_ease_of_use(path: Path, body: str, fm: dict) -> dict:
    ...
    breakdown["metadata_quality"] = (
        meta_score,
        ", ".join(meta_notes) if meta_notes else "Complete metadata",
    )
    ...
    breakdown["terminology"] = (
        max(0, term_score),
        ", ".join(term_notes) if term_notes else "Consistent terminology",
    )
...
def score_utility(path: Path, body: str, fm: dict) -> dict:
    ...
    if re.search(
        r"(?i)(alternatively|or use|another approach|you can also|extend|customize|modify|adapt)",
        body,
    ):
        freedom_score += 1
...
def score_spec_compliance(path: Path, body: str, fm: dict) -> dict:
    ...
    if "model" in fm:
        model = fm["model"]
        if model not in ["inherit", "sonnet", "haiku", "opus"] and not str(model).startswith(
            "claude-"
        ):
            opt_score -= 1
...
def score_writing_style(path: Path, body: str, fm: dict) -> dict:
    ...
    has_imperative = any(
        re.search(rf"(?m)^\s*\d+\.\s*{v}", body, re.IGNORECASE) for v in imperative_verbs
    )
...
def calculate_modifiers(path: Path, body: str, fm: dict) -> dict:
    ...
    if (
        "i can" in desc_lower
        or "i will" in desc_lower
        or "you can" in desc_lower
        or "you should" in desc_lower
    ):
    ...
    if has_dynamic_injection:
        ...
        modifiers["dynamic_injection"] = (
            +1,
            f"Uses preprocessing injection ({injection_count} directives)",
        )
    ...
    if constraint_words > 15:
        anti_patterns_found.append(
            f"over-constrained ({constraint_words} MUST/NEVER/ALWAYS — reduces flexibility)"
        )
    ...
    if not has_trigger_cue and len(desc) > 20:
        anti_patterns_found.append(
            "missing trigger phrase in description — autonomous activation impossible"
        )
    ...
    placeholder_count = sum(
        len(re.findall(re.escape(tok), body, re.IGNORECASE)) for tok in placeholder_tokens
    ...
    has_cross_ref = bool(
        re.search(r"(?i)(see also|related skill|sibling|cross-reference|companion)", body)
    )
    ...
    if refs_dir.exists():
        ...
        if ref_files:
            modifiers["supporting_files"] = (
                +1,
                f"Has references/ with {len(ref_files)} substantial files",
            )
    ...
    for field_name, value in fm.items():
        if field_name in AGENT_FIELDS:
            ...
            if expected_type == "string" and not isinstance(value, str):
                errors.append(
                    f"[agent] '{field_name}' must be a string, got: {type(value).__name__}"
                )
            elif expected_type == "integer" and not isinstance(value, int):
                errors.append(
                    f"[agent] '{field_name}' must be an integer, got: {type(value).__name__}"
                )
            elif expected_type == "boolean" and not isinstance(value, bool):
                errors.append(
                    f"[agent] '{field_name}' must be a boolean, got: {type(value).__name__}"
                )
            elif expected_type == "array" and not isinstance(value, list):
                errors.append(
                    f"[agent] '{field_name}' must be an array, got: {type(value).__name__}"
                )
            elif expected_type == "object" and not isinstance(value, dict):
                errors.append(
                    f"[agent] '{field_name}' must be an object, got: {type(value).__name__}"
                )
            ...
            if is_plugin_agent and field_name in AGENT_PLUGIN_RESTRICTED:
                warnings.append(
                    f"[agent] '{field_name}' is not supported in plugin agents (ignored by runtime)"
                )

        elif field_name in INVALID_AGENT_FIELDS:
            errors.append(
                f"[agent] Invalid field '{field_name}': {INVALID_AGENT_FIELDS[field_name]}"
            )
        elif field_name in DEPRECATED_AGENT_FIELDS:
            warnings.append(
                f"[agent] Deprecated field '{field_name}': {DEPRECATED_AGENT_FIELDS[field_name]}"
            )
        else:
...
def validate_frontmatter(
    path: Path, fm: dict, tier: str = TIER_STANDARD
    ...
    if tier == TIER_MARKETPLACE:
        ...
        if "description" not in fm:
            warnings.append(
                "[frontmatter] Missing recommended field: 'description' (recommended by Anthropic spec)"
            )
    ...
    if "name" in fm:
        ...
        if not name:
            ...
            if not re.match(r"^[a-z0-9][a-z0-9-]*[a-z0-9]$", name) and len(name) > 1:
                warnings.append(
                    f"[frontmatter] 'name' should be kebab-case (lowercase + hyphens): {name}"
                )
        ...
        if not desc:
            ...
            if not RE_DESCRIPTION_TRIGGER_WITH.search(desc):
                if tier == TIER_ENTERPRISE:
                    ...
                    infos.append(
                        "[frontmatter] Consider adding 'Trigger with ...' phrase to description"
                    )
            ...
            if RE_FIRST_PERSON.search(desc):
                if tier == TIER_ENTERPRISE:
                    ...
                    warnings.append(
                        "[frontmatter] 'description' uses first person - third person recommended"
                    )
            ...
            if RE_SECOND_PERSON.search(desc):
                if tier == TIER_ENTERPRISE:
                    ...
                    warnings.append(
                        "[frontmatter] 'description' uses second person - third person recommended"
                    )
    ...
    if "user-invocable" in fm:
        ...
        if not isinstance(ui, bool):
            errors.append(
                f"[frontmatter] 'user-invocable' must be boolean, got: {type(ui).__name__}"
            )
    ...
    if "when_to_use" in fm:
        ...
        if not isinstance(wtu, str):
            errors.append(
                f"[frontmatter] 'when_to_use' must be a string, got: {type(wtu).__name__}"
            )
        else:
            wtu_str = wtu.strip()
    ...
    if "hooks" in fm:
        ...
        if not isinstance(hooks_val, dict):
            errors.append(
                f"[frontmatter] 'hooks' must be a mapping, got: {type(hooks_val).__name__}"
            )
...
def validate_body(
    ...
    if tier == TIER_ENTERPRISE:
        if not purpose_text:
            warnings.append(
                "[body] Missing purpose statement near the top (marketplace quality standard)"
            )
        else:
            ...
            if purpose_location is not None and purpose_location > 120:
                warnings.append(
                    "[body] Purpose statement appears late in the document - keep it near the top"
                )
    ...
    for pattern, desc in time_patterns:
        ...
        for m in matches:
            warnings.append(
                f"[body] Time-sensitive information found: '{m.group()}' ({desc}) - may become stale"
            )
    ...
    for i, block in enumerate(code_blocks):
        ...
        if "set -e" not in block and "|| " not in block and "if [" not in block:
            if len(block.strip().splitlines()) > 15:  # Only warn for substantial scripts
                if re.search(r"\b(rm|mv|cp|curl|wget|pip|npm)\b", block):
                    warnings.append(
                        f"[scripts] Code block {i+1}: Consider adding error handling (set -e or || exit)"
                    )
        ...
        for num in magic_numbers[:3]:  # Limit warnings
            ...
            if not re.search(rf"#.*{num}", block):  # No comment explaining it
                warnings.append(
                    f"[scripts] Code block {i+1}: Magic number '{num}' - add comment explaining why"
                )
...
def validate_relative_links(path: Path, body: str) -> Tuple[List[str], List[str]]:
    ...
    for match in RE_RELATIVE_MD_LINK.finditer(filtered_body):
        ...
        try:
            ...
            errors.append(
                f"[relative-link] Link escapes skill directory: [{link_text}]({link_target})"
            )
            continue
    ...
    if assets_readme.exists():
        try:
            ...
            checkbox_pattern = re.compile(
                r"-\s*\[[ xX]\]\s*([^\s:]+\.(?:md|yaml|json|py|sh|template))"
            )
            matches = checkbox_pattern.findall(content)
...
def validate_supporting_files(path: Path) -> Tuple[List[str], List[str]]:
    ...
    if not refs_dir.exists():
        warnings.append(
            "[supporting] Missing references/ directory — create it for progressive disclosure"
        )
...
def validate_plugin(plugin_dir: Path, tier: str = TIER_STANDARD) -> Dict[str, Any]:
    ...
    if commands_dir.exists():
        ...
        if cmd_files:
            infos.append(
                f"[plugin] commands/ directory has {len(cmd_files)} files — consider migrating to skills/"
            )
...
def populate_compliance_db(
    ...
    c.execute(
        "CREATE INDEX IF NOT EXISTS idx_plugin_compliance_run_id ON plugin_compliance(run_id)"
    )
    ...
    for result in skill_results:
        ...
        anthropic_fields = len(
            [k for k in fm if k in SKILL_FIELDS and SKILL_FIELDS[k].get("source") == "anthropic"]
        )
        enterprise_fields = len(
            [k for k in fm if k in SKILL_FIELDS and SKILL_FIELDS[k].get("source") == "enterprise"]
        )
        ...
        if body_for_stub:
            ...
            _db_placeholder_density = (
                _db_placeholder_count / _db_word_count if _db_word_count > 0 else 0.0
            )
            if _db_lines < 30:
        ...
        ref_file_count = (
            len(list((skill_dir / "references").glob("*")))
        ...
        gold_components = sum(
            [1, has_prd, has_ard, has_refs, has_errors_md, has_examples_md, has_impl_md, has_config]
        )
    ...
    if agent_results:
        for result in agent_results:
            ...
            invalid_agent_set = set(DEPRECATED_AGENT_FIELDS.keys()) | set(
                INVALID_AGENT_FIELDS.keys()
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Print per-file OK lines and grades"
    )
    parser.add_argument(
        "--standard",
        action="store_true",
    ...
    if args.path:
        ...
        if target.is_dir():
            ...
            print(
                f"ERROR: Expected a SKILL.md, .md file, or plugin directory: {args.path}",
                file=sys.stderr,
            )
            return 1
        ...
        if target.name == "SKILL.md":
            ...
            if args.deep and target.name == "SKILL.md":
                try:
                    ...
                    from deep_eval.reporter import format_json, format_terminal
    ...
    for skill in skills:
        ...
        if verbose and not has_issues and not result.get("infos") and not args.json:
            print(
                f"✅ {rel} - {letter} ({score}/100) ({result['word_count']} words, {result['line_count']} lines)"
            )
    ...
    if args.populate_db:
        try:
            populate_compliance_db(
                args.populate_db,
                json_skill_results,
                agent_results=json_agent_results,
    ...
    if args.deep and skills:
        try:
            from deep_eval.db import populate_deep_eval_db
            from deep_eval.engine import DeepEvalEngine
            from deep_eval.reporter import (
                format_html,
                format_json,
                format_markdown,
                format_terminal,
            ...
            if deep_eval_skills:
                ...
                if args.report_format == "json":
                    ...
                    print(
                        format_terminal(deep_results, deep_summary, deep_rankings, verbose=verbose)
                    )
