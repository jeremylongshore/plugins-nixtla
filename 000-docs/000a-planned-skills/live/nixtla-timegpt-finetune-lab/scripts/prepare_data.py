...
def load_and_preprocess_data(file_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        raise ValueError(
            "Could not convert 'ds' column to datetime. Ensure it's in a recognizable format."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_output", default="train_data.csv", help="Path to save training data."
    )
    parser.add_argument(
        "--val_output", default="val_data.csv", help="Path to save validation data."
    )
    ...
    try:
        ...
        print(
            f"Data prepared successfully. Training data shape: {train_data.shape}, Validation data shape: {val_data.shape}"
        )
