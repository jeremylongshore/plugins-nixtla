...
def analyze_event_impact(
    ...
    try:
        ...
        forecast = client.forecast(
            df=prices_df_reset[['unique_id', 'ds', 'y']],
            h=len(prices_df_reset),
            freq='D'
        )

        # Merge forecast with original data
        adjusted_forecast = pd.merge(
            prices_df_reset,
            forecast,
            on=['unique_id', 'ds'],
            how='left'
...
def main():
    ...
    parser.add_argument('--prices', type=str, required=True,
                        help='Path to configured prices CSV')
    parser.add_argument('--events', type=str, required=True,
                        help='Path to events CSV')
    parser.add_argument('--niter', type=int, default=1000,
                        help='MCMC iterations for CausalImpact (default: 1000)')
    parser.add_argument('--window-days', type=int, default=3,
                        help='Event window size in days (default: 3)')
    parser.add_argument('--output-impact', type=str, default='impact_results.csv',
                        help='Path to save impact results (default: impact_results.csv)')
    parser.add_argument('--output-forecast', type=str, default='adjusted_forecast.csv',
                        help='Path to save adjusted forecast (default: adjusted_forecast.csv)')
    parser.add_argument('--output-summary', type=str, default='causal_summary.txt',
