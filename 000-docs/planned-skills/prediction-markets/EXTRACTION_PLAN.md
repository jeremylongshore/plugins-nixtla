...
- `scripts/prepare_data.py` - Data loading and validation (lines 40-167)
- `scripts/batch_forecast.py` - Main forecasting engine (lines 174-417)
- `scripts/generate_report.py` - Report generator (lines 424-541)

### nixtla-contract-schema-mapper (1 script)
- `scripts/transform_data.py` - Schema transformation and validation (lines 41-184)

### nixtla-correlation-mapper (5 scripts)
- `scripts/prepare_data.py` - Data prep and pivoting (lines 40-147)
- `scripts/correlation_analysis.py` - Correlation calculation (lines 154-310)
- `scripts/hedge_recommendations.py` - Hedging strategies (lines 317-509)
- `scripts/visualize.py` - Heatmaps and plots (lines 516-674)
- `scripts/generate_report.py` - Report generation (lines 681-789)

### nixtla-event-impact-modeler (3 scripts)
- `scripts/load_data.py` - Data loading (lines 40-113)
- `scripts/configure_model.py` - Model configuration (lines 120-205)
- `scripts/analyze_impact.py` - Event impact analysis (lines 212-337)

### nixtla-forecast-validator (1 script)
- `scripts/validate_forecast.py` - Complete validation pipeline (lines 159-413)

### nixtla-liquidity-forecaster (3 scripts)
- `scripts/fetch_data.py` - Polymarket API fetcher (lines 36-105)
- `scripts/preprocess_data.py` - Data preprocessing (lines 112-206)
- `scripts/forecast_liquidity.py` - Forecasting execution (lines 213-320)

### nixtla-market-risk-analyzer (4 scripts)
- `scripts/prepare_data.py` - Data preparation (lines 38-154)
- `scripts/risk_metrics.py` - VaR, volatility, drawdown (lines 161-431)
- `scripts/position_sizing.py` - Position size calculations (lines 438-665)
- `scripts/generate_report.py` - Risk report generation (lines 672-884)

### nixtla-model-selector (4 scripts)
- `scripts/load_and_analyze.py` - Data loading and analysis (lines 40-125)
- `scripts/select_model.py` - Model selection logic (lines 132-196)
- `scripts/execute_forecast.py` - Forecast execution (lines 203-282)
- `scripts/generate_output.py` - Output generation (lines 289-335)

### nixtla-polymarket-analyst (4 scripts)
- `scripts/fetch_contract.py` - Polymarket data fetcher (lines 36-146)
- `scripts/transform_data.py` - Data transformation (lines 153-248)
- `scripts/forecast_contract.py` - TimeGPT forecasting (lines 255-503)
- `scripts/analyze_polymarket.py` - Complete pipeline (lines 510-644)
