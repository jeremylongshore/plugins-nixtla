...
def select_model(df: pd.DataFrame, inferred_freq: str) -> Tuple[str, str]:
    ...
    try:
        ...
        if len(first_series) >= 24:  # At least 2 periods for decomposition
            ...
            print(f"Seasonality check (period={period}): {'Present' if seasonality_present else 'Not detected'}")
    except Exception as e:
    ...
    try:
        if model_name == "StatsForecast":
            ...
            raise ValueError(
                ...
                f"Must be 'StatsForecast' or 'TimeGPT'."
            )
...
Examples:
  ...
  python model_selector.py --input data.csv --visualize
    ...
    parser.add_argument(
        ...
        type=int,
        default=14,
        help='Forecast horizon in periods (default: 14)'
    )
    parser.add_argument(
        '--visualize',
        action='store_true',
        help='Generate visualization plot of input data'
    )
