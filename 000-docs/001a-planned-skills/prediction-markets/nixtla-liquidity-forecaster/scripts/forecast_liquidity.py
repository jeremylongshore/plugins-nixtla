...
def forecast_liquidity(df: pd.DataFrame, horizon: int) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if not all(col in df.columns for col in ["unique_id", "ds", "y"]):
            raise ValueError(
                "DataFrame must contain 'unique_id', 'ds', and 'y' columns."
            )
        ...
        forecast = client.forecast(
            df=df, h=horizon, freq="T"
        )  # Assuming 1 minute frequency
...
def generate_report(
    preprocessed_data: pd.DataFrame, horizon: int, output_files: dict
) -> None:
...
def main():
    ...
    parser.add_argument(
        "--horizon", type=int, required=True, help="The forecast horizon."
    )
    parser.add_argument(
        "--output",
