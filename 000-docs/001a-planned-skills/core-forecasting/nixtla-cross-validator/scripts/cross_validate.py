...
def cross_validate(
    ...
    if model_name not in ['timegpt', 'arima', 'ets', 'theta', 'naive']:
        raise ValueError(f"Invalid model name: {model_name}. Supported models are 'timegpt', 'arima', 'ets', 'theta', 'naive'.")
    ...
    for i in range(folds):
        ...
        try:
            if model_name == 'timegpt':
                ...
                forecast = forecast.rename(columns={'unique_id': 'unique_id', 'ds': 'ds', 'TimeGPT': 'y_hat'})

            elif model_name == 'arima':
                sf = StatsForecast(models=[AutoARIMA()], freq=freq, n_jobs=-1)
                forecast = sf.forecast(df=train_df, h=window_size)
                forecast = forecast.reset_index().rename(columns={'unique_id': 'unique_id', 'ds': 'ds', 'AutoARIMA': 'y_hat'})

            elif model_name == 'ets':
                sf = StatsForecast(models=[AutoETS()], freq=freq, n_jobs=-1)
                forecast = sf.forecast(df=train_df, h=window_size)
                forecast = forecast.reset_index().rename(columns={'unique_id': 'unique_id', 'ds': 'ds', 'AutoETS': 'y_hat'})

            elif model_name == 'theta':
                sf = StatsForecast(models=[AutoTheta()], freq=freq, n_jobs=-1)
                forecast = sf.forecast(df=train_df, h=window_size)
                forecast = forecast.reset_index().rename(columns={'unique_id': 'unique_id', 'ds': 'ds', 'AutoTheta': 'y_hat'})

            elif model_name == 'naive':
                sf = StatsForecast(models=[SeasonalNaive()], freq=freq, n_jobs=-1)
                forecast = sf.forecast(df=train_df, h=window_size)
                forecast = forecast.reset_index().rename(columns={'unique_id': 'unique_id', 'ds': 'ds', 'SeasonalNaive': 'y_hat'})
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', required=True, choices=['timegpt', 'arima', 'ets', 'theta', 'naive'],
